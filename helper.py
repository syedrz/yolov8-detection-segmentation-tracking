...
def _display_detected_frames(conf, model, st_frame, image, is_display_tracking=None, tracker=None):
    ...
    if is_display_tracking:
        ...
        res = model.predict(image, conf=conf, line_width=1)
...
def upload_video(conf, model):
    #st.write('key : ', key)
    #if 'video_path' not in st.sidebar:
    #    st.sidebar['video_path'] = None
    ...
    if uploaded_file is not None:
        ...
        with open(uploaded_file.name, 'rb') as video_file:
            video_bytes = video_file.read()
        if video_bytes:
            st.video(video_bytes)
